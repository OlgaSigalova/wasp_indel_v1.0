...
class OutputWriter(object):
    ...
    def __init__(
        ...
        self.header = (
            ...
            "TEST.SNP.ALT.ALLELE TEST.SNP.GENOTYPE TEST.SNP.HAPLOTYPE PHASING.BLOCK "
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        action='store_true',
        default=False,
        help="only calcualte the allele-specific counts per peak"
    )
    parser.add_argument(
        "--variants", required=True, help="Input variant count file."
    )
    parser.add_argument(
        "--bam", default = "", help="Input variant BAM file."
    )
    parser.add_argument(
    ...
    if not args.as_only:
        ...
        if not os.path.isfile(args.bam):
            raise ValueError("BAM file does not exist or not provided")
    ...
    for test in regions.get_tests():
        ...
        # Get total counts (if not allele-specific only), otherwise set total counts to 0
        if not args.as_only:
